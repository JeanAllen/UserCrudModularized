...
class User:
    ...
    def get_one(cls, data):
        query = "SELECT id, first_name, last_name, email, created_at, updated_at FROM users WHERE id=%(id_nums)s;"
        print(query)
    ...
    def update_user(cls, data):
        query = "UPDATE users SET first_name=%(fname)s, last_name=%(lname)s, email=%(email)s, updated_at=now() WHERE id=%(id_nums)s;"
        print(query)
        return connectToMySQL('User_cr').query_db(query, data)
